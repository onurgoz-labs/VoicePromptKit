...
```
.voicepromptkit/<basename>/run-NNN/inline-suggestions.md
```
...
```markdown
# VoicePromptKit Overlay — <basename>
...
```
.voicepromptkit/<basename>/run-NNN/decisions.jsonl
```
...
The third VoicePromptKit artefact (alongside `inline-suggestions.md` and
`decisions.jsonl`). Sits at `.voicepromptkit/<basename>/pronunciations.md` —
...
Every audit re-discovers the same TR pronunciation hints (Peugeot → "pöjo",
...
|---|---|---|---|---|---|---|---|---|
| `Peugeot` | pronounce | pöjo | — | French brand. Turkish TTS often reads 'Peugeot' letter-literally; the intended French reading is closer to 'pöjo'. | run-001 (2026-05-27) | run-003 (2026-05-30) | 1.3 | T1@run-001, T1@run-002, T1@run-003 |
...
| `Versailles` | rephrase | — | Versay Sarayı | Author marked this as high risk; rephrase when possible. | run-001 (2026-05-27) | run-003 (2026-05-30) | — | seed |
...
# Generated by VoicePromptKit. Override entries by hand if your TTS provider
...
(Author-managed. VoicePromptKit never touches anything below this heading.)

<!-- voicepromptkit:custom-additions:start -->
<!-- Append your own pronunciation rows here. They are preserved across runs. -->
<!-- voicepromptkit:custom-additions:end -->
````
...
PRESERVED VERBATIM on every rebuild. VoicePromptKit never touches it.
