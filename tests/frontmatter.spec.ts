...
describe('parsePromptFile', () => {
  ...
  it('applies env-var overrides for unspecified fields (env < frontmatter precedence)', () => {
    const raw = `Just a body\n`;
    const env = {
      PROMPTCHECKER_TARGET_MODEL: 'gpt-4o',
      PROMPTCHECKER_OUTPUT: 'markdown,json',
      PROMPTCHECKER_EXPAND_COUNT: '8',
      PROMPTCHECKER_EXECUTOR: 'mcp-codex',
    };
    const { frontmatter } = parsePromptFile(raw, env);
    expect(frontmatter.target_model).toBe('gpt-4o');
    expect(frontmatter.output).toEqual(['markdown', 'json']);
    expect(frontmatter.expand_count).toBe(8);
    expect(frontmatter.executor).toBe('mcp-codex');
  });

  it('frontmatter beats env vars when both set', () => {
    const raw = `---\ntarget_model: claude-haiku-4-5\n---\nbody`;
    const env = { PROMPTCHECKER_TARGET_MODEL: 'gpt-4o' };
    const { frontmatter } = parsePromptFile(raw, env);
    expect(frontmatter.target_model).toBe('claude-haiku-4-5');
  });
});
