...
Apply the four detection categories from `tr_phonetic_ref`. Every emitted finding gets `fix_kind: "advisory"` — there are no other fix kinds in this lens.
...
Every finding is `fix_kind: "advisory"`. Populate exactly one of `suggested_fix` / `pronunciation_entry`:

- `number_readability` + `punctuation` → `suggested_fix` populated, `pronunciation_entry: null`
- `abbreviation` + `foreign_word` → `pronunciation_entry` populated, `suggested_fix: null`

**Never write `fix_kind: "replace"` or `fix_kind: "pronunciation_hint"`** — those were removed in v0.4 and PromptChecker never auto-applies TR suggestions. Every TR finding is advisory.
